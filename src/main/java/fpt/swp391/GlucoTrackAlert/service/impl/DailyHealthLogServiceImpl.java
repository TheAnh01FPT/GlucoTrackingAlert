...
public class DailyHealthLogServiceImpl implements DailyHealthLogService {
    ...
    public Page<DailyHealthLogResponse> getLogs(Long patientId, Pageable pageable) {
        ...
        // Resolve thresholds grouped by (patientId, patientType, metricType) to avoid
        // N+1
        ...
        for (DailyHealthLog log : page.getContent()) {
            ...
            String key = (pId == null ? "null" : pId.toString()) + "|" + (pType == null ? "" : pType) + "|"
                    + MetricType.BLOOD_SUGAR.name();
    ...
    public List<DailyHealthLogResponse> getChartData(Long patientId, LocalDate from, LocalDate to) {
        List<DailyHealthLog> logs = dailyHealthLogRepository.findByPatientIdAndLogDateBetweenOrderByLogDate(patientId,
                from, to);
    ...
    private void triggerDailyAiPrediction(Patient patient, DailyHealthLog log) {
        ...
        new Thread(() -> {
            try {
                ...
                int marriedVal = patient.getEverMarried() != null && patient.getEverMarried().equalsIgnoreCase("Yes")
                        ? 1
                        : 0;
                ...
                if (response.statusCode() == 200) {
                    ...
                    String aiSummary = "Dựa trên mô hình học máy Random Forest phân tích chỉ số hôm nay, nguy cơ xảy ra biến chứng đột quỵ của bạn là "
                            + String.format("%.2f", riskPercentage) + "% (Mức độ: " + riskLevel + ").";
    ...
    public void assessWeeklyRisk(Long patientId) {
        ...
        for (DailyHealthLog log : weeklyLogs) {
            if (log.getBloodSugar() != null) {
                ...
                String sugarStatus = healthThresholdService.evaluate(sugarVal, patientId, patientType,
                        MetricType.BLOOD_SUGAR);
                if (sugarStatus.contains("HIGH")) {
                    highSugarDays++;
                }
                if (!"NORMAL".equals(sugarStatus) && !"UNKNOWN".equalsIgnoreCase(sugarStatus)
                        && !"unknown".equalsIgnoreCase(sugarStatus)) {
                    warningCount++;
                }
            }
            ...
            if (log.getSystolic() != null) {
                ...
                String sysStatus = healthThresholdService.evaluate(BigDecimal.valueOf(sysVal), patientId, patientType,
                        MetricType.SYSTOLIC);
                if (!"NORMAL".equals(sysStatus) && !"UNKNOWN".equalsIgnoreCase(sysStatus)
                        && !"unknown".equalsIgnoreCase(sysStatus)) {
                    warningCount++;
                }
            }
            ...
            if (log.getDiastolic() != null) {
                ...
                String diaStatus = healthThresholdService.evaluate(BigDecimal.valueOf(diaVal), patientId, patientType,
                        MetricType.DIASTOLIC);
                if (!"NORMAL".equals(diaStatus) && !"UNKNOWN".equalsIgnoreCase(diaStatus)
                        && !"unknown".equalsIgnoreCase(diaStatus)) {
                    warningCount++;
                }
            }
        ...
        if (!existing.isEmpty() && !existingReport.isEmpty()) {
            ...
            // If the latest update in logs is not after the assessment time, and we have
            // exactly 1 record, we don't need to recalculate
            if (assessedAt != null && maxUpdatedAt != null && !maxUpdatedAt.isAfter(assessedAt)
                    && existing.size() == 1) {
                needUpdate = false;
            }
        }
        ...
        if (needUpdate) {
            ...
            try {
                ...
                if (response.statusCode() == 200) {
                    ...
                    String aiSummary = "Dựa trên mô hình học máy Random Forest phân tích chỉ số trung bình tuần này, nguy cơ xảy ra biến chứng đột quỵ của bạn là "
                            + String.format("%.2f", riskPercentage) + "% (Mức độ: " + riskLevel + ").";
                    ...
                    // Averages
                    BigDecimal avgSugarVal = BigDecimal.valueOf(avgSugarMmol).setScale(2,
                            java.math.RoundingMode.HALF_UP);
                    BigDecimal avgSystolicVal = countSystolic > 0
                            ? BigDecimal.valueOf(sumSystolic / countSystolic).setScale(2,
                                    java.math.RoundingMode.HALF_UP)
                            : null;
                    BigDecimal avgDiastolicVal = countDiastolic > 0
                            ? BigDecimal.valueOf(sumDiastolic / countDiastolic).setScale(2,
                                    java.math.RoundingMode.HALF_UP)
                            : null;
                    BigDecimal avgSleepVal = countSleep > 0
                            ? BigDecimal.valueOf(sumSleep / countSleep).setScale(2, java.math.RoundingMode.HALF_UP)
                            : null;
                    BigDecimal avgWaterVal = countWater > 0
                            ? BigDecimal.valueOf(sumWater / countWater).setScale(2, java.math.RoundingMode.HALF_UP)
                            : null;
                    ...
                    if (prevSugar != null) {
                        ...
                        if (prevSugar.compareTo(BigDecimal.ZERO) > 0) {
                            sugarChangePercent = sugarChange.multiply(BigDecimal.valueOf(100)).divide(prevSugar, 2,
                                    java.math.RoundingMode.HALF_UP);
                        }
                    }
