...
public class DailyHealthLogServiceImpl implements DailyHealthLogService {
    ...
    public Page<DailyHealthLogResponse> getLogs(Long patientId, Pageable pageable) {
        ...
        Map<String, Optional<fpt.swp391.GlucoTrackAlert.model.healthlog.HealthThreshold>> resolved = new HashMap<>();
        ...
        List<DailyHealthLogResponse> mapped = page.getContent().stream().map(log -> {
            ...
            Optional<fpt.swp391.GlucoTrackAlert.model.healthlog.HealthThreshold> opt = resolved.get(key);
            String bloodSugarStatus = evaluateStatus(log.getBloodSugar(), opt);

            Optional<fpt.swp391.GlucoTrackAlert.model.healthlog.HealthThreshold> optSys = resolved.get((log.getPatient() != null ? log.getPatient().getId().toString() : "null") + "|" + (log.getPatient() != null ? log.getPatient().getPatientType() : "") + "|" + MetricType.SYSTOLIC.name());
            String systolicStatus = evaluateStatus(log.getSystolic() != null ? java.math.BigDecimal.valueOf(log.getSystolic()) : null, optSys);

            Optional<fpt.swp391.GlucoTrackAlert.model.healthlog.HealthThreshold> optDia = resolved.get((log.getPatient() != null ? log.getPatient().getId().toString() : "null") + "|" + (log.getPatient() != null ? log.getPatient().getPatientType() : "") + "|" + MetricType.DIASTOLIC.name());
            ...
            if (log.getBloodSugar() != null) {
                if (opt.isPresent()) {
                    fpt.swp391.GlucoTrackAlert.model.healthlog.HealthThreshold t = opt.get();
    ...
    private String evaluateStatus(java.math.BigDecimal value, Optional<fpt.swp391.GlucoTrackAlert.model.healthlog.HealthThreshold> opt) {
        String status = "unknown";
        if (value == null) {
            return status;
        }
    ...
    public DailyHealthLogResponse createLog(Long patientId, DailyHealthLogRequest request) {
        ...
        // 3 lời gọi dưới đây đều chạy NỀN (@Async, xem bên trong từng service) —
        // không chặn response trả về cho người dùng, và mỗi hàm tự try/catch lỗi
        // của riêng nó nên không cần bọc try/catch ở đây nữa.
    ...
    public DailyHealthLogResponse updateLog(Long id, DailyHealthLogRequest request) {
        ...
        // 3 lời gọi dưới đây đều chạy NỀN (@Async) — trước đây đây chính là
        // phần khiến trang "Sửa nhật ký" bị chậm, vì phải đợi gửi email SMTP
        // + gọi AI dự đoán (Flask) xong hết mới trả kết quả. Giờ response trả
        // về ngay sau khi lưu DB, các việc phụ hoàn tất sau đó vài giây.
